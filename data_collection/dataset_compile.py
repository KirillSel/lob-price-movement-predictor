...
# file path
input_path = './data_binance_SOLBTC/' 
...
for subdir, dirs, files in os.walk(input_path):
    for file in files:
        if file.endswith((".csv")):
            ...
            # read csv data
            binance_solbtc = pd.read_csv(data_path, names=header_list)
            binance_solbtc.sort_values('timestamp', inplace=True)

            # reorder columns and filter
            binance_solbtc = binance_solbtc[new_lob_order].dropna()

            # normalize each row
            df_array = binance_solbtc.values
            norm_df_array = []
            ...
            np.save(output_path + '/validation/' + filename +
                    '.npy', np_norm_array[split_row:, :])